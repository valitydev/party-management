...
COMPOSE_DEPENDENCIES_LIST := $(subst !, ,$(COMPOSE_DEPENDENCIES))
WAIT_FOR_DEPS := $(foreach dep,$(COMPOSE_DEPENDENCIES_LIST),wait-for $(dep) -t 0 --)

DOCKER_WC_OPTIONS := -v $(PWD):$(PWD) --workdir $(PWD)
DOCKER_WC_EXTRA_OPTIONS ?= --rm
DOCKER_RUN = $(DOCKER) run -t $(DOCKER_WC_OPTIONS) $(DOCKER_WC_EXTRA_OPTIONS)

DOCKERCOMPOSE_RUN = $(DOCKERCOMPOSE_W_ENV) run --rm $(DOCKER_WC_OPTIONS) $(TEST_CONTAINER_NAME) $(WAIT_FOR_DEPS)
