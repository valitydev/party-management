...
handle_function(Func, Args0, Opts) ->
    scoper:scope(
        partymgmt,
        fun() ->
            handle_function_(Func, filter_user_info_arg(Func, Args0), Opts)
        end
    ).
...
filter_user_info_arg(Func, Args0)when
    Func =:= 'ComputeProviderTerminal'
->
    Args0;
filter_user_info_arg(_Func, Args0) ->
